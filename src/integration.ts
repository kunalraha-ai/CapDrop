...
// ─── Error pattern recognizers ────────────────────────────────────────────────
interface ErrorPattern {
  name: string;
  pattern: RegExp;
  suggestedFix: (match: RegExpMatchArray) => string;
}
...
const ERROR_PATTERNS: ErrorPattern[] = [
  {
    ...
    suggestedFix: ([, code, msg]) =>
      `Fix TypeScript error TS${code}: ${msg}. Check type declarations and ensure correct types are used.`,
  },
  {
    name: "Module not found",
    pattern: /Cannot find module '(.+?)'/i,
    suggestedFix: ([, mod]) =>
      `Install missing module: npm install ${mod.replace(/^@types\//, "@types/")}. Or check import path.`,
  },
  {
    name: "npm build failure",
    pattern: /npm ERR! code (\w+)/i,
    suggestedFix: ([, code]) =>
      `npm error code ${code}. Try: npm ci to reinstall all deps, or check package.json for invalid scripts.`,
  },
  {
    name: "Rust compile error",
    pattern: /^error\[E(\d+)\]:\s*(.+)/m,
    suggestedFix: ([, code, msg]) =>
      `Fix Rust error E${code}: ${msg}. Run: cargo check for a detailed trace.`,
  },
  {
    name: "Webpack missing dependency",
    pattern: /Module not found: Error: Can't resolve '(.+?)'/i,
    suggestedFix: ([, dep]) =>
      `Webpack cannot resolve '${dep}'. Install with: npm install ${dep} --save or --save-dev.`,
  },
  {
    name: "Permission denied",
    pattern: /EACCES:\s*permission denied/i,
    suggestedFix: () =>
      `Permission denied. Check file ownership and ensure the workspace has write access.`,
  },
  {
    name: "Port in use",
    pattern: /EADDRINUSE.*:(\d+)/i,
    suggestedFix: ([, port]) =>
      `Port ${port} is in use. Kill the occupying process or change the port in .env.`,
  },
];
...
export function analyzeErrors(stderr: string, stdout: string): AnalyzedError[] {
...
export async function runIntegrationLoop(
  ...
  const sessionId = context.globalState.get<string>("activeSessionId") ?? "integration-session";
  const persona = "integration";

  vscode.window.showInformationMessage(
    `🔗 Integration Expert: Starting build loop for "${buildCommand}"`
  );
  ...
  for (let attempt = 1; attempt <= MAX_CORRECTION_LOOPS; attempt++) {
    ...
    // ── Run command ──────────────────────────────────────────────────────────
    const result = await runAndDisplay(buildCommand, workspaceRoot, persona, sessionId);
    ...
    writeExecutionTrace(workspaceRoot, trace);

    // ── Success path ─────────────────────────────────────────────────────────
    if (result.exitCode === 0) {
      updateIntentLogStatus(workspaceRoot, "approved");
      vscode.window.showInformationMessage(
        `✅ Integration Expert: Build succeeded on ${header}!`
      );
      return true;
    }
    ...
    if (errors.length === 0 && attempt < MAX_CORRECTION_LOOPS) {
      ...
      vscode.window.showWarningMessage(
        `⚠️ ${header}: Build failed with exit ${result.exitCode}. No known patterns matched. Retrying...`
      );
      continue;
    }
    ...
    if (attempt < MAX_CORRECTION_LOOPS) {
      const action = await vscode.window.showWarningMessage(
        `🔧 ${header}: Build failed. Integration Expert found ${errors.length} error(s).\n\n${suggestionSummary}`,
        { modal: false },
        "Retry Build",
        "Stop"
      );
  ...
  // ── Loop exhausted ───────────────────────────────────────────────────────
  vscode.window.showErrorMessage(
    `🚫 Integration Expert: Reached maximum ${MAX_CORRECTION_LOOPS} correction attempts. Human intervention required.\n\nCheck ".gemini/terminal_traces.json" for full trace history.`
  );
  return false;
}

// ─── Load and display last terminal traces ────────────────────────────────────
export function getLastTraces(workspaceRoot: string, limit = 5): ExecutionTrace[] {
  const tracePath = path.join(workspaceRoot, ".gemini", "terminal_traces.json");
  if (!fs.existsSync(tracePath)) {
    return [];
  }
  try {
    const all: ExecutionTrace[] = JSON.parse(fs.readFileSync(tracePath, "utf8"));
